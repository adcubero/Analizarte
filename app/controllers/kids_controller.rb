class KidsController < ApplicationController
  ...
  def create
    ...
    @kid.tutor_id = Tutor.where(["email = ?", params[:tutor_email]]).first.id
    puts Tutor.where(["email = ?", params[:tutor_email]]).first.email
  ...
  private
    ...
    def kid_params
      params.require(:kid).permit(:email, :password, :name, :age, :tutor_email)
    end
end
